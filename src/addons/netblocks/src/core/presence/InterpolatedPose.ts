...
export class InterpolatedPose {
  private _prev?: {ts: number; snapshot: PoseSnapshot};
  private _next?: {ts: number; snapshot: PoseSnapshot};
  ...
  sample(now: number): PoseSnapshot {
    ...
    if (t < 0) t = 0;
    if (t > 1 + MAX_EXTRAPOLATION) t = 1 + MAX_EXTRAPOLATION;

    // Head
    _pos.lerpVectors(
      this._prev.snapshot.head.position,
      this._next.snapshot.head.position,
      t
    );
    _quat
...
function copyHand(src: HandPose, dst: HandPose): void {
...
function copySnapshot(src: PoseSnapshot, dst: PoseSnapshot): void {
